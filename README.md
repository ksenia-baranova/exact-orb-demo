...
реализованы. Отдельный обязательный тест импортных границ handler ещё
остаётся открытым пунктом его формальной приёмки.
...
| **PII в DEBUG-журнале.** Полный диагностический вывод содержит дату рождения, координаты и карту целиком | риск принят явно; блокер публичного развёртывания |
