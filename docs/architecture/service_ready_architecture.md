...
решения о версии. Отдельный golden и покомпонентные проверки
`CalculationVersion` живут в `tests/test_calculation_version.py`; там же
интеграционно доказан промах общего кэша при другой версии. Механизм реализован,
а его обязательное application wiring остаётся за точкой композиции C3.
...
Четыре расхождения между рамкой и кодом на дату документа. Ни одно не является
...
2. **`RunContext` лежит не там, где ранние компонентные документы.**
...
3. **Фиктивный шов в `birth`.** `exact_orb.birth.__init__` реэкспортирует
...
4. **Приватность логов пока очищена только на расчётном пути.** `calculate_natal`
