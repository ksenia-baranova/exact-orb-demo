...
**Статус:** промты 1–3 выполнены; ветка готова к потреблению M1-6.
...
`ChartArtifactResolver.drain()` был единственным намеренным изменением поведения
существующего calculation-компонента в промте 1. Унаследованный тест промта 2 затем
потребовал симметричного cancellation cleanup того же lifecycle seam; остальные
calculation semantics и компоненты не менялись, а `engine.py` получил только уточнение
module docstring.
...
`aclose() → drain()` и последующей ошибке leader; минимальная симметричная
