...
**Ревизия:** 2026-09-12 — добавлен целевой, ещё не реализованный поток
неизвестного времени по ADR-0032.
...
**Целевое расширение ADR-0032 — принято, не реализовано.**
...
только birth/timezone-слоем. Текущая модель из восьми полей остаётся
фактической до отдельного implementation-этапа.
...
По принятому, но ещё не реализованному ADR-0032 `CalculationInput` получает
`birth_time_domain_digest: str | None`. Это меняет key schema и префикс на
`eo:calc:v2:`; текущий `eo:calc:v1:` действует до реализации. Полные
...
этим решением заменена. Отдельные предупреждения ADR-0008 о смене знака и
направления остаются самостоятельным пробелом. Ни аспектная проверка, ни эти
предупреждения в текущем коде не реализованы.
...
**Целевое дополнение ADR-0032 — не реализовано.** В ветке
