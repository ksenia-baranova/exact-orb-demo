...
расчётную основу и повторно не планируются.
...
**Синхронизация документов выполнена:** в ревизии 1.1
`docs/requirements/build_chart/exact_orb_negative_corner_scenarios.md`
`BuildAttempt` и `build_revision` уже помечены отложенными. Диаграммы
прежней модели вынесены в `sequence_diagrams/deferred/build_attempt/`.
...
5. **Каталог мест — частично закрыто.** JSONL-формат,
   `LocalPlaceCatalog.from_file` и `scripts/build_place_catalog.py` реализованы.
   Остаются объём и поставка рабочего набора, поиск подсказок и подключение
   к UI — M1-5 roadmap; loader повторно не проектируется.
