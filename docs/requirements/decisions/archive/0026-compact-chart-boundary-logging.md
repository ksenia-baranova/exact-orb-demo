...
Дата: 2026-09-09.
**Статус: заменено ADR-0028.**

ADR-0028 отменяет summary-режим и требование единственной полной копии:
локальная диагностика снова получает полный вход и выход каждой границы.
Настоящий файл сохранён как история решения. Введённые здесь поля envelope
`calculation_key`, `status`, `payload_mode` и `message_type` продолжают
действовать в форме, определённой ADR-0028.
