...
Статус документа: рабочий, версия 2.2 (2026-09-07).
...
Ревизия 2026-09-07: механизм `CalculationVersion` приведён к реализации A2;
его application wiring оставлен за C3.
...
Механизм девятикомпонентного отпечатка реализован; его однократное вычисление и
передача в application resolver остаются за общей точкой композиции C3.
...
станций (ADR-0001). Самая дорогая операция системы: итеративный поиск станций
и точных дат.
**Статус:** реализовано.
...
компонент, `run_id`, доступный `calculation_key` и режим payload. Путь Build
Natal по ADR-0025/0026 сохраняет все входящие и исходящие boundary-события,
но большие внутренние выходы пишет как summary; полный расчётный результат
остаётся один раз на выходе handler. Это сознательная диагностика локального
стенда; она сохраняет birth-data и итоговую карту за пределами TTL сессии.
Маскирование либо отдельный защищённый маршрут для этих событий блокирует
...
**Отдельными задачами:** маскирование логов (блокирует публичное размещение);
