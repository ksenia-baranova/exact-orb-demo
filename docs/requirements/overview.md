...
компонент, а путь Build Natal по ADR-0025 пишет на DEBUG полный однострочный
JSON входящих и исходящих сообщений. Это сознательная диагностика локального
стенда; она сохраняет birth-data и полный расчётный результат за пределами
TTL сессии. Маскирование либо отдельный защищённый маршрут для этих событий
блокирует публичное размещение.
