...
нет. Research Corpus хранит разрешённую проекцию бессрочно по ADR-0023; будущий
сырой consented-корпус требует отдельного согласия. Полный локальный DEBUG-журнал
является принятым исключением по ADR-0025/0028, описанным в §4.12.
...
пределами TTL сессии. До публичного размещения требуется маскирование либо
отдельный защищённый маршрут для этих событий.
**Статус:** полный локальный журнал реализован; privacy-hardening отложен.
...
первый UI; отображение рассчитанной карты; деплой и браузерная приёмка.
...
**Граница публичного запуска.** Требования §7 и подготовки логов выполняются
до публичного доступа независимо от номера этапа. Исторические замеры
расчётов и выполненный benchmark SQLite P4 сохраняются как факт; актуальные
natal/cosmogram/transit и целевая серверная нагрузка требуют отдельной проверки.
