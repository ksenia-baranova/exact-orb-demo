...
Ниже DEBUG payload и logging-проекции не сериализуются. Это сознательная
диагностика локального стенда; она сохраняет birth-data и итоговую карту за
пределами TTL сессии. Маскирование либо отдельный защищённый маршрут для этих
событий блокирует публичное размещение.
