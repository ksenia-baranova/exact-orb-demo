...
FIELD_START = re.compile(r"(?<!\S)([a-z][a-z0-9_]*)=")
...
def _read_value(text: str) -> object:
    """Read key=value output without requiring Python vs JSON scalar spelling."""
...
def _assert_record(
    ...
    name, body = record.getMessage().strip().split(maxsplit=1)
    assert name == event
    matches = list(FIELD_START.finditer(body))
    assert matches and not body[: matches[0].start()].strip()
    actual: dict[str, object] = {}
    for index, match in enumerate(matches):
        key = match.group(1)
        assert key not in actual, f"Duplicate field: {key}"
        end = matches[index + 1].start() if index + 1 < len(matches) else len(body)
        text = body[match.end() : end].strip()
        assert text, f"Missing value: {key}"
        actual[key] = _read_value(text)
...
def test_result_terminal_fields_and_levels_for_consistent_statuses(
    ...
    assert operation_logging.log_operation_finished(**arguments) is None
    _assert_record(
...
def test_cancelled_terminal_omits_result_fields_with_result_positive_control(
    ...
    assert operation_logging.log_operation_finished(**result_arguments) is None
    _assert_record(
        caplog, "application_operation_finished", logging.INFO,
...
def test_payload_keywords_are_rejected_after_valid_call_positive_control(
    ...
    values = {"run_id": RUN_ID, **arguments}
