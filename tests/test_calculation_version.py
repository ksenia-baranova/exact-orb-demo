...
# Captured before ADR-0032 and unchanged by later engine-only method fixes;
# AspectConfig/ConfigurationConfig/StrengthConfig remain byte-for-byte stable.
...
def test_orb_boundary_fix_bumps_only_engine_version_without_changing_profiles(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ephe = _ephemeris_directory(tmp_path / "ephe", {})
    ...
    previous = record.model_copy(update={"engine_version": "5"})

    assert record.engine_version == "6"
